...
<%
    ...
    Concert concertObj = (Concert)request.getAttribute("concertObj");
...
<jsp:include page="../header.jsp"></jsp:include>
				    		...
				    		<div class="row">
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					<tr class="success bold"><td>序号</td><td>订单id</td><td>下单用户</td><td>演唱会名称</td><td>订单数量</td><td>订单总金额</td><td>订单状态</td><td>下单时间</td><td>收货人</td><td>收货人电话</td><td>操作</td></tr>
				    					<% 
				    						/*计算起始序号*/
 										...
 										<tr>
 											...
 											<td><%=orderInfo.getConcertObj().getConcertName() %></td>
 											<td><%=orderInfo.getOrderCount() %></td>
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="orderInfoQueryForm" id="orderInfoQueryForm" action="<%=basePath %>OrderInfo/frontlist" class="mar_t15" method="post">
            ...
            <div class="form-group">
            	<label for="concertObj_concertId">演唱会名称：</label>
                <select id="concertObj_concertId" name="concertObj.concertId" class="form-control">
                	<option value="0">不限制</option>
	 				<%
	 				for(Concert concertTemp:concertList) {
	 					String selected = "";
 					if(concertObj!=null && concertObj.getConcertId()!=null && concertObj.getConcertId().intValue()==concertTemp.getConcertId().intValue())
 						selected = "selected";
	 				%>
 				 <option value="<%=concertTemp.getConcertId() %>" <%=selected %>><%=concertTemp.getConcertName() %></option>
	 				<%
	 				}
	 				%>
 			</select>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" style="height:450px; overflow: scroll;">
		  ...
		  <div class="form-group">
		  	 <label for="orderInfo_concertObj_concertId_edit" class="col-md-3 text-right">演唱会名称:</label>
		  	 <div class="col-md-9">
			    <select id="orderInfo_concertObj_concertId_edit" name="orderInfo.concertObj.concertId" class="form-control">
			    </select>
		  	 </div>
		  </div>
		  <div class="form-group">
		  	 <label for="orderInfo_orderCount_edit" class="col-md-3 text-right">订单数量:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="orderInfo_orderCount_edit" name="orderInfo.orderCount" class="form-control" placeholder="请输入订单数量">
			 </div>
		  </div>
		  <div class="form-group">
		  ...
		  <div class="form-group">
		  	 <label for="orderInfo_address_edit" class="col-md-3 text-right">收货人地址:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="orderInfo_address_edit" name="orderInfo.address" class="form-control" placeholder="请输入收货人地址">
			 </div>
		  </div>
		  <div class="form-group">
	...
	$.ajax({
		...
		success : function (orderInfo, response, status) {
			if (orderInfo) {
				...
				$.ajax({
					url: basePath + "Concert/listAll",
					type: "get",
					success: function(concerts,response,status) { 
						$("#orderInfo_concertObj_concertId_edit").empty();
						var html="";
		        		$(concerts).each(function(i,concert){
		        			html += "<option value='" + concert.concertId + "'>" + concert.concertName + "</option>";
		        		});
		        		$("#orderInfo_concertObj_concertId_edit").html(html);
		        		$("#orderInfo_concertObj_concertId_edit").val(orderInfo.concertObjPri);
					}
				});
				$("#orderInfo_orderCount_edit").val(orderInfo.orderCount);
