...
public class OrderInfoController extends BaseController {
	...
	@InitBinder("concertObj")
	public void initBinderconcertObj(WebDataBinder binder) {
		binder.setFieldDefaultPrefix("concertObj.");
	}
	...
	public void userAdd(OrderInfo orderInfo, BindingResult br,
		...
		int concertId = orderInfo.getConcertObj().getConcertId();
		float price = concertService.getConcert(concertId).getConcertPrice();
	...
	public void list(@ModelAttribute("userObj") UserInfo userObj,@ModelAttribute("concertObj") Concert concertObj,String orderStateObj,String orderTime,String receiveName,String telephone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(userObj, concertObj, orderStateObj, orderTime, receiveName, telephone, page);
	    /*计算总的页数和总的记录数*/
	    orderInfoService.queryTotalPageAndRecordNumber(userObj, concertObj, orderStateObj, orderTime, receiveName, telephone);
	    /*获取到总的页码数目*/
	...
	public String frontlist(@ModelAttribute("userObj") UserInfo userObj,@ModelAttribute("concertObj") Concert concertObj,String orderStateObj,String orderTime,String receiveName,String telephone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (orderStateObj == null) orderStateObj = "";
		if (orderTime == null) orderTime = "";
		if (receiveName == null) receiveName = "";
		if (telephone == null) telephone = "";
		List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(userObj, concertObj, orderStateObj, orderTime, receiveName, telephone, currentPage);
	    /*计算总的页数和总的记录数*/
	    orderInfoService.queryTotalPageAndRecordNumber(userObj, concertObj, orderStateObj, orderTime, receiveName, telephone);
	    /*获取到总的页码数目*/
	    int totalPage = orderInfoService.getTotalPage();
	    ...
	    request.setAttribute("concertObj", concertObj);
	...
	public String userFrontlist(@ModelAttribute("userObj") UserInfo userObj,@ModelAttribute("concertObj") Concert concertObj,String orderStateObj,String orderTime,String receiveName,String telephone,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (orderStateObj == null) orderStateObj = "";
		if (orderTime == null) orderTime = "";
		if (receiveName == null) receiveName = "";
	...
	public void OutToExcel(@ModelAttribute("userObj") UserInfo userObj,@ModelAttribute("concertObj") Concert concertObj,String orderStateObj,String orderTime,String receiveName,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(orderStateObj == null) orderStateObj = "";
        if(orderTime == null) orderTime = "";
        if(receiveName == null) receiveName = "";
        if(telephone == null) telephone = "";
        List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(userObj,concertObj,orderStateObj,orderTime,receiveName,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "OrderInfo信息记录"; 
        String[] headers = { "订单id","下单用户","演唱会名称","订单数量","订单总金额","订单状态","下单时间","收货人","收货人电话"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<orderInfoList.size();i++) {
        	OrderInfo orderInfo = orderInfoList.get(i); 
        	dataset.add(new String[]{orderInfo.getOrderId() + "",orderInfo.getUserObj().getName(),orderInfo.getConcertObj().getConcertName(),orderInfo.getOrderCount() + "",orderInfo.getTotalMoney() + "",orderInfo.getOrderStateObj(),orderInfo.getOrderTime(),orderInfo.getReceiveName(),orderInfo.getTelephone()});
        }
        /*
